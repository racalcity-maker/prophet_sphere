...
static uint16_t sine_wave_u16(uint32_t now_ms, uint32_t period_ms)
{
    ...
    float c = cosf(phase * 6.28318530718f); /* 2*pi */
    /* Breath profile: start from 0 -> peak -> back to 0 in one full cycle. */
    float n = 0.5f - 0.5f * c;              /* [0..1], minima at cycle boundaries */
    /* Shape curve: near peak changes slower, near floor changes faster. */
                                 ...
                                 bool dimmed)
{
    ...
    for (uint32_t y = 0; y < LED_MATRIX_H; ++y) {
        for (uint32_t x = 0; x < LED_MATRIX_W; ++x) {
            float fx = (float)x - cx;
            float fy = (float)y - cy;
            float r_norm = sqrtf(fx * fx + fy * fy) / max_r; /* [0..1] */
            if (r_norm > 1.0f) {
                r_norm = 1.0f;
            }
            ...
            float mix = (v1 + v2 + v3) / 3.0f;                 /* [-1..1] */
            float n = 0.5f + 0.5f * mix;                       /* [0..1] */
            float edge = smoothstepf((1.0f - r_norm) / 0.14f); /* soft edge fade */
