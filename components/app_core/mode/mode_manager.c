...
static mode_dispatch_pipeline_ctx_t s_dispatch_pipeline;
...
esp_err_t mode_manager_init(void)
{
    ...
    s_dispatch_pipeline.action_executor = &s_action_executor;
    s_dispatch_pipeline.mode_timers = &s_mode_timers;
    s_dispatch_pipeline.offline_grumble_fade_out_ms = (uint32_t)CONFIG_ORB_OFFLINE_GRUMBLE_FADE_OUT_MS;
...
esp_err_t mode_manager_perform_switch(orb_mode_t target_mode)
{
    ...
    mode_switch_orchestrator_ctx_t switch_ctx = {
        .action_executor = &s_action_executor,
        .mode_timers = &s_mode_timers,
        .runtime_hook = runtime_hook,
    };
    ESP_RETURN_ON_ERROR(mode_switch_orchestrator_run(&switch_ctx,
                                                     previous_mode,
                                                     target_mode,
                                                     previous_desc,
                                                     target_desc),
                        TAG,
                        "mode switch orchestrator failed");
...
esp_err_t mode_manager_dispatch_event(const app_event_t *event)
{
    ...
    return mode_dispatch_pipeline_run(&s_dispatch_pipeline, current_mode, mode, event);
}
